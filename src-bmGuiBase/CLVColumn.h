//*** LICENSE ***
// ColumnListView, its associated classes and source code, and the other components of Santa's Gift
// Bag are being made publicly available and free to use in freeware and shareware products with a
// price under $25 (I believe that shareware should be cheap). For overpriced shareware (hehehe) or
//commercial products, please contact me to negotiate a fee for use. After all, I did work hard on
// this class and invested a lot of time into it. That being said, DON'T WORRY I don't want much. It
// totally depends on the sort of project you're working on and how much you expect to make off it.
// If someone makes money off my work, I'd like to get at least a little something.  If any of the
// components of Santa's Gift Bag are is used in a shareware or commercial product, I get a free
// copy.  The source is made available so that you can improve and extend it as you need. In general
// it is best to customize your ColumnListView through inheritance, so that you can take advantage
// of enhancements and bug fixes as they become available. Feel free to distribute the
// ColumnListView source, including modified versions, but keep this documentation and license with
// it.
...
// Flags
enum {
	CLV_COLTYPE_STATICTEXT = 0x00000000,
	CLV_COLTYPE_BITMAP = 0x00000001,
	CLV_COLTYPE_USERTEXT = 0x00000002,
	CLV_COLTYPE_MASK = 0x00000003,
	// bitmask used by column-types

	CLV_SORT_KEYABLE = 0x00000004,
	// Can be used as the sorting key
	CLV_NOT_MOVABLE = 0x00000008,
	// Column can't be moved by user
	CLV_NOT_RESIZABLE = 0x00000010,
	// Column can't be resized by user
	CLV_LOCK_AT_BEGINNING = 0x00000020,
	// Movable columns may not be placed or moved by the user
	// into a position before this one
	CLV_LOCK_AT_END = 0x00000040,
	// Movable columns may not be placed or moved by the user
	// into a position after this one
	CLV_HIDDEN = 0x00000080,
	// This column is hidden initially
	CLV_MERGE_WITH_RIGHT = 0x00000100,
	// Merge this column label with the one that follows it.
	CLV_LOCK_WITH_RIGHT = 0x00000200,
	...
	CLV_PUSH_PASS = 0x00000800,
	// Causes this column, if pushed by an expander to the
	// left, to pass that push on and also push the next
	// column to the right.
	CLV_HEADER_TRUNCATE = 0x00001000,
	// Causes this column label to be tructated with an ellipsis
	// if the column header is narrower than the text it contains.
	CLV_TELL_ITEMS_WIDTH = 0x00002000,
	// Causes items in this column to be informed when the column
	// width is changed.  This is necessary for CLVEasyItems.
	CLV_RIGHT_JUSTIFIED = 0x00004000,
	// Causes the column, when resized, to shift its content,
	...
	// added by zooey:
	CLV_COLDATAMASK = 0x00038000,
	// bitmap used by column-datatypes
	CLV_COLDATA_STRING = 0x00000000,
	// Column is a string, used in sorting. N.B.: this is the default
	CLV_COLDATA_NUMBER = 0x00008000,
	// Column is a number, used in sorting
	CLV_COLDATA_DATE = 0x00010000,
	// Column is a date, used in sorting
	CLV_COLDATA_BIGTIME = 0x00020000
	// Column is a bigtime, used in sorting
};
...
void GetTruncatedString(const char* full_string, char* truncated, float width,
	int32 truncate_buf_size, const BFont* font);
...
public:
	...
	*/
	...
	virtual void SetWidth(float width);	 // Can be overridden to detect changes to the column width
										 ...
										 // column widths
	uint32 Flags() const;
	inline uint32 Type() const { return fFlags & CLV_COLTYPE_MASK; }
	bool IsShown() const;
	...
	int8 BitmapOffset() const { return fBitmapOffset; }
	void BitmapOffset(uint8 o) { fBitmapOffset = o; }
	...
	//   BPoint(text_point.x-1,text_point.y+2.0) to
