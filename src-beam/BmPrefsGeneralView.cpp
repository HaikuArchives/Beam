	...
	,	mMailboxPanel( NULL)
{
			...
			new HGroup( 
				new VGroup( 
					new MBorder( M_LABELED_BORDER, 10, (char*)"Status Window",
						new HGroup( 
							new VGroup(
								mDynamicStatusWinControl = new BmCheckControl( 
									"Only show active jobs", 
								   new BMessage(BM_DYNAMIC_STATUS_WIN_CHANGED), 
								   this, ThePrefs->GetBool("DynamicStatusWin")
								),
								new Space( minimax(0,4,0,4)),
								mMailMoverShowControl = new BmTextControl( 
									"Time before mail-moving-job will be shown (ms):", 
									false, 5
								),
								mPopperRemoveControl = new BmTextControl( 
									"Time before mail-receiving-job will be removed "
									"(ms):", false, 5
								),
								mSmtpRemoveControl = new BmTextControl( 
									"Time before mail-sending-job will be removed (ms):", 
									false, 5
								),
								mRemoveFailedControl = new BmTextControl( 
									"Time before a failed job will be removed (ms):", 
									false, 5
								),
								new Space( minimax(0,4,0,4)),
								0
					...
					new MBorder( 
						M_LABELED_BORDER, 10, 
						(char*)"Performance & Timing Options",
						new HGroup( 
							new VGroup(
								mCacheRefsOnDiskControl = new BmCheckControl( 
									"Cache mailfolders (on disk)", 
						 			new BMessage(BM_CACHE_REFS_DISK_CHANGED), 
						 			this, ThePrefs->GetBool("CacheRefsOnDisk")
						 		),
								mCacheRefsInMemControl = new BmCheckControl( 
									"Keep mailfolders in memory once loaded", 
							 		new BMessage(BM_CACHE_REFS_MEM_CHANGED), 
							 		this, ThePrefs->GetBool("CacheRefsInMem")
							 	),
								mNetBufSizeSendControl = new BmTextControl( 
									"Network buffer size when sending mail (bytes):", 
									false, 5
								),
								mNetRecvTimeoutControl = new BmTextControl( 
									"Timeout for network-connections (seconds):", 
									false, 5
								),
								0
							),
				...
				new MBorder( M_LABELED_BORDER, 10, (char*)"General GUI Options",
					new HGroup(
						new VGroup(
							...
							new Space( minimax(0,10,0,10)),
							mRestoreFolderStatesControl = new BmCheckControl( 
								"Restore mailfolder-view state on startup", 
						 		new BMessage(BM_RESTORE_FOLDER_STATES_CHANGED), 
						 		this, ThePrefs->GetBool("RestoreFolderStates")
						 	),
							mInOutAtTopControl = new BmCheckControl( 
								"Show in & out - folders at top of mailfolder-view", 
							...
							new Space( minimax(0,10,0,10)),
							mShowToolbarIconsControl = new BmCheckControl( 
								"Show Icons in Toolbar-Buttons", 
							...
							new Space( minimax(0,10,0,10)),
							mCloseViewWinControl = new BmCheckControl( 
								"Close Mail-Window Upon Reply", 
	...
	val = BmString("") << ThePrefs->GetInt("MSecsBeforeRemoveFailed", 
														5000*1000)/1000;
...
void BmPrefsGeneralView::Initialize() {
	...
	TheBubbleHelper->SetHelp( 
		mDynamicStatusWinControl, 
		"Determines the layout of the job-status window.\n"
		"\tif checked, the job-status window will only display "
		"currently active jobs (it will grow/shrink accordingly)\n"
		"\tif unchecked, the job-status window will be static "
		"and show all known jobs, even if they are not active."
	);
	TheBubbleHelper->SetHelp( 
		mMailMoverShowControl, 
		"Here you can enter the time (in ms) Beam will let pass before it\n"
		"will show the GUI for a mail-moving operation inside the job-window."
	);
	TheBubbleHelper->SetHelp( 
		mPopperRemoveControl, 
		"Here you can enter the time (in ms) Beam will let a "
		"finished POP3-job linger inside the job-window\n"
		"(this way you can check the results before the job is removed)."
	);
	TheBubbleHelper->SetHelp( 
		mSmtpRemoveControl, 
		"Here you can enter the time (in ms) Beam will let a "
		"finished SMTP-job linger inside the job-window \n"
		"(this way you can check the results before the job is removed)."
	);
	TheBubbleHelper->SetHelp( 
		mRemoveFailedControl, 
		"Here you can enter the time (in ms) Beam will keep "
		"any failed job inside the job-status window\n"
		"(this way you can see that a problem occurred "
		"before the job is removed)."
	);
	TheBubbleHelper->SetHelp( 
		mWorkspaceControl, 
	...
	TheBubbleHelper->SetHelp( 
		mRestoreFolderStatesControl, 
		"Checking this makes Beam remember the state of the "
		"mailfolder-view \n(which of the folders are expanded/collapsed).\n"
		"If unchecked, Beam will always start with a collapsed mailfolder-view."
	);
	TheBubbleHelper->SetHelp( 
		mInOutAtTopControl, 
		"Determines whether the in-, out- & drafts-folders will be shown\n"
	...
	TheBubbleHelper->SetHelp( 
		mUseDeskbarControl, 
		"Checking this makes Beam show an icon \nin the Deskbar."
	);
	TheBubbleHelper->SetHelp( 
		mBeepNewMailControl, 
		"Checking this makes Beam play the 'New E-mail' beep-event\n"
	...
	TheBubbleHelper->SetHelp( 
		mCacheRefsOnDiskControl, 
		"Checking this will cause Beam to cache \n"
		"mail-folder contents on disk.\n\n"
		"Doing this speeds up the display \n"
		"of a mail-folder's contents quite a lot."
	);
	TheBubbleHelper->SetHelp( 
		mCacheRefsInMemControl, 
		"Checking this will cause Beam to keep \n"
	...
	TheBubbleHelper->SetHelp( 
		mNetBufSizeSendControl, 
		"Here you can enter the network buffer size (in bytes)\n"
		"Beam will use for outgoing connections.\n\n"
		"If sending seems slow, try a larger value in here."
	);
	TheBubbleHelper->SetHelp( 
		mNetRecvTimeoutControl, 
		"Here you can enter the time (in ms) Beam\n"
		"will wait for an answer from a remote network-server."
	);
	TheBubbleHelper->SetHelp( 
		mShowToolbarIconsControl, 
		"If checked, Beam will display icons in each toolbar-button."
	);
	TheBubbleHelper->SetHelp( 
	...
	TheBubbleHelper->SetHelp( 
		mShowAlertForErrorsControl, 
		"If unchecked, Beam will not display error-messages on-screen,\n"
		"but they will just be logged (written to a log-file) instead.\n"
		"If in doubt, leave this checked."
	);
	TheBubbleHelper->SetHelp( 
		mCloseViewWinControl, 
		"If you check this, Beam will automatically close\n"
		"the mail-view-window if you reply to that mail."
	);
...
void BmPrefsGeneralView::Update() {
	...
	mDynamicStatusWinControl->SetValueSilently( 
		ThePrefs->GetBool("DynamicStatusWin")
	);
	mRestoreFolderStatesControl->SetValueSilently( 
		ThePrefs->GetBool("RestoreFolderStates")
	);
	mInOutAtTopControl->SetValueSilently( 
		ThePrefs->GetBool("InOutAlwaysAtTop", false)
	);
	mListviewLikeTrackerControl->SetValueSilently( 
		ThePrefs->GetBool("ListviewLikeTracker", false)
	);
	mUseDeskbarControl->SetValueSilently( 
		ThePrefs->GetBool("UseDeskbar", true)
	);
	mBeepNewMailControl->SetValueSilently( 
		ThePrefs->GetBool("BeepWhenNewMailArrived", true)
	);
	mShowTooltipsControl->SetValueSilently( 
		ThePrefs->GetBool("ShowTooltips", true)
	);
	mCacheRefsOnDiskControl->SetValueSilently( 
		ThePrefs->GetBool("CacheRefsOnDisk")
	);
	mCacheRefsInMemControl->SetValueSilently( 
		ThePrefs->GetBool("CacheRefsInMem")
	);
	mShowToolbarIconsControl->SetValueSilently( 
		ThePrefs->GetBool("ShowToolbarIcons", true)
	);
	mShowToolbarBorderControl->SetValueSilently( 
		ThePrefs->GetBool("ShowToolbarBorder", true)
	);
	mShowAlertForErrorsControl->SetValueSilently( 
		ThePrefs->GetBool("ShowAlertForErrors", true)
	);
	mCloseViewWinControl->SetValueSilently( 
		ThePrefs->GetBool("CloseViewWinAfterMailAction", true)
	);
	BmString val;
	val << ThePrefs->GetInt("MSecsBeforeMailMoverShows")/1000;
		...
		switch( msg->what) {
			case BM_TEXTFIELD_MODIFIED: {
				...
				if ( source == mMailMoverShowControl)
					ThePrefs->SetInt(
						"MSecsBeforeMailMoverShows", 
						1000*atoi(mMailMoverShowControl->Text())
					);
				else if ( source == mPopperRemoveControl)
					ThePrefs->SetInt(
						"MSecsBeforePopperRemove", 
						1000*atoi(mPopperRemoveControl->Text())
					);
				else if ( source == mSmtpRemoveControl)
					ThePrefs->SetInt(
						"MSecsBeforeSmtpRemove", 
						1000*atoi(mSmtpRemoveControl->Text())
					);
				else if ( source == mRemoveFailedControl)
					ThePrefs->SetInt(
						"MSecsBeforeRemoveFailed", 
						1000*atoi(mRemoveFailedControl->Text())
					);
				else if ( source == mNetBufSizeSendControl)
					ThePrefs->SetInt(
						"NetSendBufferSize", 
						atoi(mNetBufSizeSendControl->Text())
					);
				else if ( source == mNetRecvTimeoutControl)
					ThePrefs->SetInt(
						"ReceiveTimeout", 
						atoi(mNetRecvTimeoutControl->Text())
					);
				NoticeChange();
				break;
			}
			case BM_RESTORE_FOLDER_STATES_CHANGED: {
				ThePrefs->SetBool(
					"RestoreFolderStates", 
					mRestoreFolderStatesControl->Value()
				);
				NoticeChange();
				break;
			}
			case BM_DYNAMIC_STATUS_WIN_CHANGED: {
				ThePrefs->SetBool(
					"DynamicStatusWin", 
					mDynamicStatusWinControl->Value()
				);
				NoticeChange();
				break;
			}
			...
			case BM_BEEP_NEW_MAIL_CHANGED: {
				ThePrefs->SetBool(
					"BeepWhenNewMailArrived", 
					mBeepNewMailControl->Value()
				);
				NoticeChange();
				break;
			}
			...
			case BM_SHOW_TOOLBAR_ICONS_CHANGED: {
				ThePrefs->SetBool(
					"ShowToolbarIcons", 
					mShowToolbarIconsControl->Value()
				);
				NoticeChange();
				break;
			}
			...
			case BM_SHOW_ALERTS_FOR_ERRORS_CHANGED: {
				ThePrefs->SetBool(
					"ShowAlertForErrors", 
					mShowAlertForErrorsControl->Value()
				);
				TheLogHandler->ShowErrorsOnScreen( 
					mShowAlertForErrorsControl->Value()
				);
				NoticeChange();
				break;
			...
			case BM_CLOSE_VIEWWIN_CHANGED: {
				ThePrefs->SetBool(
					"CloseViewWinAfterMailAction", 
					mCloseViewWinControl->Value()
				);
				NoticeChange();
				break;
			}
