...
#define SIEVE_VERSION "libSieve: CMU Sieve 2.1"
...
extern int sieve_interp_alloc(sieve_interp_t **interp, void *interp_context);
extern int sieve_interp_free(sieve_interp_t **interp);

/* add the callbacks for actions. undefined behavior results if these
   are called after sieve_script_parse is called! */
extern int sieve_register_redirect(sieve_interp_t *interp, sieve_callback *f);
extern int sieve_register_discard(sieve_interp_t *interp, sieve_callback *f);
extern int sieve_register_reject(sieve_interp_t *interp, sieve_callback *f);
extern int sieve_register_fileinto(sieve_interp_t *interp, sieve_callback *f);
extern int sieve_register_keep(sieve_interp_t *interp, sieve_callback *f);
extern int sieve_register_vacation(sieve_interp_t *interp, sieve_vacation_t *v);
extern int sieve_register_imapflags(sieve_interp_t *interp, sieve_imapflags_t *mark);
extern int sieve_register_notify(sieve_interp_t *interp, sieve_callback *f);

/* add the callbacks for messages. again, undefined if used after
   sieve_script_parse */
extern int sieve_register_size(sieve_interp_t *interp, sieve_get_size *f);
extern int sieve_register_header(sieve_interp_t *interp, sieve_get_header *f);
extern int sieve_register_envelope(sieve_interp_t *interp, sieve_get_envelope *f);

typedef int sieve_parse_error(int lineno, const char *msg, 
			      void *interp_context,
			      void *script_context);
extern int sieve_register_parse_error(sieve_interp_t *interp, sieve_parse_error *f);

typedef int sieve_execute_error(const char *msg, void *interp_context,
				void *script_context, void *message_context);
extern int sieve_register_execute_error(sieve_interp_t *interp, 
				 sieve_execute_error *f);
 
/* given an interpretor and a script, produce an executable script */
extern int sieve_script_parse(sieve_interp_t *interp, FILE *script,
		       void *script_context, sieve_script_t **ret);

extern int sieve_script_free(sieve_script_t **s);

/* execute a script on a message, producing side effects via callbacks */
extern int sieve_execute_script(sieve_script_t *script, 
			 void *message_context);

/* Get space separated list of extensions supported by the implementation */
extern const char *sieve_listextensions(void);
